...
T = input("T:")
for i in range(T):
    N,K = [int(i) for i in raw_input("N, K:").split()] # get N and K on one line space-separated
    A = [int(i) for i in raw_input("A:").split()] # get the list on one line space-seperated
    A = sorted(A)   # sort list
    A = A[K:N-K]    # slice off the highest and lowest K elements from the list
    print sum(A) / float(len(A)) # find the mean of the remaining list
